...
export const getFeedV41 = functions
  ...
  .onCall(async (data, context) => {
    ...
    try {
      ...
      const safeCode = err?.details?.safeCode || err?.message || 'FEED_QUERY_FAILED';
      const errMessage = err?.message || String(err);
      console.error(`[getFeedV41:CRITICAL] build=RIPPLE-V41-FEED-RESET stage=${stage} code=${safeCode} message=${errMessage}`);
      throw new functions.https.HttpsError(
        'internal',
        typeof safeCode === 'string' ? safeCode : 'FEED_QUERY_FAILED',
        {
...
export const toggleLikeSecure = functions.https.onCall(async (data, context) => {
  ...
  await db.runTransaction(async (transaction) => {
    ...
    if (likeSnap.exists && existingReaction === reactionType) {
      ...
      const createdAt = likeSnap.exists && likeSnap.data()?.createdAt ? likeSnap.data()!.createdAt : Date.now();
