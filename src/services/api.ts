...
export async function uploadProfilePhoto(uid: string, file: File): Promise<string> {
  ...
  const photoRef = ref(storage, `profile_photos/${uid}/${safeFilename}`);
...
// Like / Unlike / React post using deterministic ID {postId}_{userId}.
export async function toggleLike(
  ...
  if (postAuthorId === 'anonymous') {
    const toggleLikeFn = httpsCallable<any, { liked: boolean; reactionType?: string }>(functions, 'toggleLikeSecure');
    const res = await toggleLikeFn({ postId, reactionType });
    return { liked: res.data.liked, reactionType: res.data.reactionType || reactionType };
  }
  ...
  await runTransaction(db, async (transaction) => {
    ...
    const existingReaction = likeSnap.exists() ? (likeSnap.data().reactionType || 'like') : null;

    if (likeSnap.exists() && existingReaction === reactionType) {
      // Unlike / remove reaction if same reaction clicked again
      ...
      // Set or update reaction, preserving existing createdAt if present
      const existingCreatedAt = likeSnap.exists() && likeSnap.data().createdAt ? likeSnap.data().createdAt : Date.now();
      ...
      if (postAuthorId !== currentUser.uid) {
        ...
        if (notifSnap.exists()) {
          transaction.update(notifRef, { reactionType });
        } else {
