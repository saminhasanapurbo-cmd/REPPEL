...
import { toggleFollow, blockUser, unblockUser, getUserCounts, recordProfileView, uploadProfilePhoto } from '../services/api';
...
export function ProfileView({
    ...
    async function loadUser() {
      ...
      try {
        ...
        if (!userDocs.empty) {
          ...
          setEditPhotoURL(uData.photoURL || '');
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    setEditError('');
    try {
      await updateProfileDetails(editDisplayName.trim(), editBio.trim(), editPhotoURL.trim() || undefined);
      if (profile) {
        setProfile({
          ...profile,
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-white rounded-3xl p-5 shadow-2xl border border-slate-100">
            ...
            <form onSubmit={handleSaveProfile} className="space-y-4">
              <div>
                <label className="text-xs font-bold text-slate-700 block mb-1.5">Profile Picture</label>
                <div className="flex items-center gap-3 mb-2">
                  <img
                    src={editPhotoURL || profile.photoURL || `https://api.dicebear.com/7.x/avataaars/svg?seed=${profile.username}`}
                    alt="Preview"
                  ...
                  <div className="flex gap-1.5 flex-wrap">
                    <button
                      type="button"
                      onClick={() => setEditPhotoURL(`https://api.dicebear.com/7.x/avataaars/svg?seed=${profile.username}`)}
                      className="px-2 py-1 text-[10px] font-bold border border-slate-200 rounded-lg hover:bg-slate-50 text-slate-700"
                    >
                      Modern
                    </button>
                    <button
                      type="button"
                      onClick={() => setEditPhotoURL(`https://api.dicebear.com/7.x/bottts/svg?seed=${profile.username}`)}
                      className="px-2 py-1 text-[10px] font-bold border border-slate-200 rounded-lg hover:bg-slate-50 text-slate-700"
                    >
                      Robot
                    </button>
                    <button
                      type="button"
                      onClick={() => setEditPhotoURL(`https://api.dicebear.com/7.x/lorelei/svg?seed=${profile.username}`)}
                      className="px-2 py-1 text-[10px] font-bold border border-slate-200 rounded-lg hover:bg-slate-50 text-slate-700"
                    >
                      Art
                    </button>
                    <button
                      type="button"
                      onClick={() => setEditPhotoURL(`https://api.dicebear.com/7.x/identicon/svg?seed=${profile.username}`)}
                      className="px-2 py-1 text-[10px] font-bold border border-slate-200 rounded-lg hover:bg-slate-50 text-slate-700"
                    >
                      Geometric
                    </button>
                ...
                >
                  {uploadingPhoto ? <Loader2 className="w-4 h-4 animate-spin text-blue-600" /> : <Upload className="w-4 h-4 text-slate-500" />}
                  <span>{uploadingPhoto ? 'Uploading...' : 'Upload Custom Image (JPEG, PNG, WebP up to 5MB)'}</span>
                </button>
              </div>
