...
export const PostCard = React.memo(function PostCard({
  ...
  const [userReactionType, setUserReactionType] = useState<string>('like');
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(doc(db, 'likes', likeDocId), (docSnap) => {
      if (docSnap.exists()) {
        setIsLiked(true);
        setUserReactionType(docSnap.data().reactionType || 'like');
      } else {
        setIsLiked(false);
  ...
  const handleLike = async () => {
    ...
    try {
      const res = await toggleLike(post.id, userProfile, post.authorId, isLiked ? userReactionType : 'like');
      setIsLiked(res.liked);
      setUserReactionType(res.reactionType);
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const handleReaction = async (reactionType: string) => {
    ...
    setShowReactionPicker(false);
    try {
      const res = await toggleLike(post.id, userProfile, post.authorId, reactionType);
      setIsLiked(res.liked);
      setUserReactionType(res.reactionType);
    } catch (err) {
      console.error(err);
  ...
  return (
    <article
      ref={cardRef}
      id={`post-${post.id}`}
      className="bg-white border-b border-slate-200/70 py-4 px-4 sm:px-5 hover:bg-slate-50/40 transition relative"
    >
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          ...
          <div className="relative flex items-center gap-1" ref={pickerRef}>
            <button
              ...
              title={isBlocked ? "Interaction blocked" : "Like post"}
            >
              {isLiked ? (
                <span className="text-sm leading-none">{REACTION_EMOJIS[userReactionType] || '❤️'}</span>
              ) : (
            ...
            <button
              id={`post-reaction-picker-trigger-${post.id}`}
              type="button"
              disabled={isBlocked}
              onClick={() => setShowReactionPicker(prev => !prev)}
              className="p-1.5 rounded-xl text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition text-xs"
            ...
            {showReactionPicker && !isBlocked && (
              <div className="absolute bottom-full mb-2 left-0 z-40 bg-white rounded-full shadow-xl border border-slate-200/90 py-1.5 px-3 flex items-center gap-2.5 animate-in fade-in zoom-in-95 duration-150">
                <button type="button" onClick={() => handleReaction('like')} className={`hover:scale-125 transition text-base p-1 rounded-full ${userReactionType === 'like' ? 'bg-rose-50 ring-1 ring-rose-300' : ''}`} title="Like" aria-label="React with Like">❤️</button>
                <button type="button" onClick={() => handleReaction('wave')} className={`hover:scale-125 transition text-base p-1 rounded-full ${userReactionType === 'wave' ? 'bg-blue-50 ring-1 ring-blue-300' : ''}`} title="Wave" aria-label="React with Wave">🌊</button>
                <button type="button" onClick={() => handleReaction('sparkle')} className={`hover:scale-125 transition text-base p-1 rounded-full ${userReactionType === 'sparkle' ? 'bg-amber-50 ring-1 ring-amber-300' : ''}`} title="Sparkle" aria-label="React with Sparkle">✨</button>
                <button type="button" onClick={() => handleReaction('rocket')} className={`hover:scale-125 transition text-base p-1 rounded-full ${userReactionType === 'rocket' ? 'bg-indigo-50 ring-1 ring-indigo-300' : ''}`} title="Rocket" aria-label="React with Rocket">🚀</button>
                <button type="button" onClick={() => handleReaction('clap')} className={`hover:scale-125 transition text-base p-1 rounded-full ${userReactionType === 'clap' ? 'bg-emerald-50 ring-1 ring-emerald-300' : ''}`} title="Clap" aria-label="React with Clap">👏</button>
              </div>
            )}
          </div>
        </div>
