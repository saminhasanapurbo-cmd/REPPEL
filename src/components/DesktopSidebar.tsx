...
export function DesktopSidebar({
  ...
  return (
    <aside 
      id="desktop-sidebar" 
      className="hidden md:flex flex-col justify-between w-60 xl:w-68 h-screen sticky top-0 px-3 xl:px-4 py-4 border-r border-slate-200/80 bg-white select-none shrink-0"
    >
      <div className="flex flex-col space-y-4">
        {/* Brand Header */}
        <div 
          onClick={() => onSelectTab('home')}
          className="flex items-center gap-2.5 px-3 py-2 rounded-xl cursor-pointer hover:bg-slate-50 transition w-fit group"
        >
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] text-slate-400 font-medium">v47</span>
          </div>
        </div>
        ...
        <nav className="space-y-1">
          ...
          {/* Admin Moderation Queue (if admin) */}
          {isAdmin && onOpenAdmin && (
            <button
              id="sidebar-nav-admin"
              type="button"
              onClick={onOpenAdmin}
              className="w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl transition text-sm font-medium text-amber-800 hover:bg-amber-50"
            >
              <div className="flex items-center gap-3">
        ...
        <div className="pt-2">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-3 px-4 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold shadow-xs transition active:scale-[0.99]"
          >
      ...
      <div className="relative pt-2 border-t border-slate-100">
        {showAccountMenu && (
          <div 
            ...
            className="absolute bottom-full left-0 mb-2 w-full bg-white rounded-2xl shadow-xl border border-slate-200/90 p-1.5 z-30 animate-in fade-in slide-in-from-bottom-2 duration-150"
          >
            ...
            <button
              ...
              className="w-full flex items-center gap-2 px-3 py-2 text-xs text-slate-700 hover:bg-slate-50 rounded-xl transition"
            >
            ...
            <button
              ...
              className="w-full flex items-center gap-2 px-3 py-2 text-xs text-rose-600 hover:bg-rose-50 rounded-xl transition"
            >
        ...
        <button
          ...
          className="w-full flex items-center justify-between p-2 rounded-xl hover:bg-slate-100/80 transition group text-left"
        >
